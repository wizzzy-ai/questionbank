...
public interface QuizResultRepository extends JpaRepository<QuizResult, Long> {
	...
	@Query("select coalesce(avg(r.percentage), 0) from QuizResult r where r.student.id = :studentId")
	double averagePercentage(@Param("studentId") Long studentId);

	@Query("select coalesce(max(r.percentage), 0) from QuizResult r where r.student.id = :studentId")
